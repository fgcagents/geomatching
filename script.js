...
// Nueva función para calcular el retraso acumulado
function getRetrasoAcumulado(trainName, horaActual) {
  const tren = itineraryList.find(t => t.Tren === trainName);
  if (!tren) return 0;

  const itinerario = getOrderedItinerary(tren);
  if (itinerario.length === 0) return 0;

  // Tiempo programado total (primera a última parada)
  const horaPrimeraParada = parseHora(itinerario[0].hora);
  const horaUltimaParada = parseHora(itinerario[itinerario.length - 1].hora);
  const tiempoProgramadoTotal = (horaUltimaParada - horaPrimeraParada) / 60000;

  // Tiempo transcurrido real desde la primera parada
  const tiempoTranscurridoReal = (horaActual - horaPrimeraParada) / 60000;

  // Calcular retraso acumulado
  const retraso = tiempoTranscurridoReal - tiempoProgramadoTotal;
  return Math.max(0, Math.round(retraso));
}
...
function updateMapMarkers() {
  ...
  let count = 0;
  const horaActual = getHoraActual(); // Obtenemos la hora actual para el cálculo del retraso
  ...
  idToTrainMap.forEach((trainData, id) => {
    ...
    if (lat && lng) {
      ...
      const retrasoAcumulado = getRetrasoAcumulado(trainData.tren, horaActual);

      let retardHTML = '';
      if (retrasoAcumulado > 0) {
        retardHTML = `<br><span class="label">Retard acumulat:</span> 
                     <span class="value" style="color:red;">+${retrasoAcumulado} min</span>`;
      } else {
      ...
      const marker = L.marker([lat, lng], {
        ...
        direction: 'top',
        offset: [4, -15],
        className: (retrasoAcumulado <= 2) // Si el retraso acumulado es <=2 minutos, se considera a tiempo
